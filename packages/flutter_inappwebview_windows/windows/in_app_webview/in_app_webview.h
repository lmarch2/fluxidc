...
namespace flutter_inappwebview_plugin
{
  ...
  public:
    ...
    // 存活标志:异步 COM 回调按值捕获此指针,执行前检查,避免 WebView 销毁后
    // 回调仍解引用悬空 this(teardown 竞态 use-after-free,崩在 createContentWorld 等处)
    std::shared_ptr<bool> aliveGuard = std::make_shared<bool>(true);
