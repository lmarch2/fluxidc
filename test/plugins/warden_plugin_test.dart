...
void main() {
  ...
  group('PluginRegistry.resolveMinPostLength', () {
    ...
    test('IDC Flare 默认不注册 warden 插件', () {
      expect(PluginRegistry.plugins.whereType<WardenPlugin>(), isEmpty);
    });

    test('IDC Flare 不应用 Linux.DO 分类专属字数规则', () {
  ...
  group('CharacterCountsOverlay', () {
    testWidgets('IDC Flare 字数不足时显示 `x / min`，不带 Linux.DO 警告', (tester) async {
